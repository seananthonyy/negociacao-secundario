...
Lógica compartilhada de orquestração do pipeline. Usado por:
  - rotinas/teste-debug.ipynb          (um bloco autocontido por script)
  - rotinas/run-pipeline-diario.ipynb  (a cadeia inteira, na ordem certa)
...
Uso típico:
    ...
    pc.RodarUltimosN(5)          # rotina diária
...
RAIZ = Path(__file__).resolve().parent.parent.parent   # a raiz do projeto
SCRIPTS = RAIZ / "codigos" / "scripts"          # um script por pasta: <n>/<n>.py


def CaminhoScript(script: str) -> Path:
    """codigos/scripts/<script>/<script>.py — cada um mora na pasta de mesmo nome."""
    return SCRIPTS / script / f"{script}.py"
...
def RodarUltimosN(n: int = 5) -> list:
    ...
    alterações retroativas). É o que o notebook diário chama."""
