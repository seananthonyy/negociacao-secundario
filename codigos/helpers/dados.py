...
ESQUEMA: dict[str, pa.Schema] = {
    ...
    # ── As quatro da CALCULADORA ────────────────────────────────────────────
    # Eram ipca.db e di.db, dois SQLite separados. Viraram Parquet em 03/09/2026,
    # junto com o resto: a base inteira passa a viver num lugar so, e no banco vai
    # para o mesmo bucket.
    #
    # Os nomes de coluna NAO seguem o prefixo vr/cd/dt do projeto porque sao
    # CONTRATO com a calculadora_rf: ela le estas tabelas pelo nome da coluna.
    # Renomear aqui quebra a precificacao inteira, em silencio.
    "IPCA": pa.schema([
        ("dtIPCA", TEXTO),              # 'AAAA-MM' (mes de referencia)
        ("vrIndiceIPCA", REAL),         # numero-indice do IBGE
        ("dtDivulgacaoIPCA", TEXTO),
    ]),
    "IPCAProjetado": pa.schema([
        ("dtIPCAProjetado", TEXTO),     # 'AAAA-MM'
        ("vrProjecaoIPCA", REAL),       # % do mes
    ]),
    "DiHistorico": pa.schema([
        ("dtReferencia", TEXTO),
        ("vrTaxaDiAnual", REAL),        # % a.a., base 252
        ("vrTaxaDiDiaria", REAL),       # fator diario
    ]),
    "CurvaDi": pa.schema([
        ("dtReferencia", TEXTO),
        ("du", INTEIRO),                # dias uteis ate o vertice
        ("diasCorridos", INTEIRO),
        ("vrTaxa", REAL),
    ]),
}
...
PARTICAO: dict[str, str | None] = {
    ...
    "Outstanding": None,
    "PuPar": "dtReferencia",
    # As quatro da calculadora sao SERIES, mas nao particionadas: somam ~16 mil
    # linhas e 0,3 MB. Particionar tabela miuda so multiplica arquivo a toa, e a
    # calc as le INTEIRAS toda vez (ela monta o acumulado do indice desde 1979).
    "IPCA": None,
    "IPCAProjetado": None,
    "DiHistorico": None,
    "CurvaDi": None,
...
CHAVE: dict[str, tuple[str, ...]] = {
    ...
    "IPCA": ("dtIPCA",),
    "IPCAProjetado": ("dtIPCAProjetado",),
    "DiHistorico": ("dtReferencia",),
    "CurvaDi": ("dtReferencia", "du"),
}
...
def NomePasta(tabela: str) -> str:
    ...
    As duas alternativas da regex existem por causa de SIGLA. A primeira corta na
    subida (`...oB...`); a segunda corta quando uma sequencia de maiusculas volta a
    virar palavra (`IPCA|Projetado`). Sem ela, `IPCAProjetado` sairia como
    `i_p_c_a_projetado` — e esses nomes viram prefixo de S3 no Athena, entao a
    feiura seria permanente."""
    return re.sub(r"(?<=[a-z0-9])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])", "_", tabela).lower()
