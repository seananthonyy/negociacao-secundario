...
# Raiz do projeto: helpers/../../ -> a pasta que contem codigos/, config/ e database/.
# A raiz do PROJETO (a pasta que contem codigos/, config/, database/, docs/).
# Tres niveis acima porque este arquivo mora em codigos/helpers/config.py.
RAIZ = Path(__file__).resolve().parent.parent.parent

# Onde moram config.toml e .env.
...
def GarantirEnv() -> None:
    ...
    if not envCarregado:
        ...
        load_dotenv(DIR_CONFIG / ".env")
        envCarregado = True


# Chaves de [paths] que NAO devem ser ancoradas na raiz. A calculadoraDir tem
# resolucao propria em calc.DirCalculadora (variavel de ambiente do banco tem
...
def GarantirCfg() -> None:
    ...
    if cfgCache is None:
        GarantirEnv()
        with open(DIR_CONFIG / "config.toml", "rb") as fh:
...
def ObterListaEmails(which: str) -> list[str]:
    """Lista de destinatários de email. Ordem de resolução:
      1. helpers/destinatarios.py (não versionado)
    ...
    try:
        import destinatarios as destinos  # helpers/ está no sys.path em runtime
...
# Objeto global exposto para importação direta: from config import cfg
class CfgProxy:
