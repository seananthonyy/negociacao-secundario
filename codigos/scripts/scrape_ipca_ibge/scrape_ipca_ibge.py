...
scrape_ipca_ibge.py
===================
IPCA realizado (número-índice) do IBGE → a tabela `IPCA`.

É um dos insumos da calculadora de renda fixa (VNA de IPCA+) — ver `codigos/helpers/calc.py`.
...
CLI:
    python codigos/scripts/scrape_ipca_ibge.py
"""
...
def Gravar(indices: dict[str, float], divulgacoes: dict[str, str], log) -> int:
    ...
    antes = D.Escalar('SELECT COUNT(*) FROM "IPCA"') or 0
    D.Upsert("IPCA", pd.DataFrame(
        [{"dtIPCA": d, "vrIndiceIPCA": v, "dtDivulgacaoIPCA": dv} for d, v, dv in linhas]))
    depois = D.Escalar('SELECT COUNT(*) FROM "IPCA"') or 0
