...
Baixa a curva pré × DI da B3 via API (sem browser). Um download, dois destinos:
  ...
  2. `data/a tabela `CurvaDi``  — a **curva inteira** (todos os vértices, du a du),
     insumo da calculadora de renda fixa para projetar/descontar fluxos DI
     (ver `codigos/helpers/calc.py`). Metade do antigo `atualizar_di.py` da calculadora;
...
CLI:
    python codigos/scripts/scrape_b3_curva_di.py --date 2026-06-05
"""
...
# Curva completa (di.db) — schema é contrato com a calculadora, ver codigos/helpers/calc.py.
...
def GravarCurvaDi(vertices: list[tuple[int, int, float]], dtStr: str, log) -> int:
    """Arquiva a curva inteira na tabela CurvaDi (insumo da calculadora)."""
    D.Upsert("CurvaDi", pd.DataFrame(
        [{"dtReferencia": dtStr, "du": du, "diasCorridos": dc, "vrTaxa": taxa}
         for du, dc, taxa in vertices]))
    log.info("curva_di: %s — %d vertices arquivados em CurvaDi", dtStr, len(vertices))
    return len(vertices)
