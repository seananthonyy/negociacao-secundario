...
Relatório HTML interativo de crédito privado com quatro visões:
  ...
  4. Boletim Diário  — o pregão inteiro, com seletor de data

Carrega todos os dados disponíveis na base (sem filtro de período).
O Boletim é a mesma agregação que o relatório diário fazia — ele foi absorvido aqui
...
# ---------------------------------------------------------------------------
# Helpers de formatação e calendário
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Boletim Diário — agrega por pregão, o pregão inteiro por ticker
# ---------------------------------------------------------------------------
...
def RenderizarHtml(
    ...
    # O template mora ao lado deste script, nao numa pasta global: e dado DELE.
    # O prefixo `template_` no nome evita confundi-lo com o HTML de saida, que
    # se chamaria igual em relatorios/.
    env  = Environment(loader=FileSystemLoader(str(Path(__file__).parent)))
    tmpl = env.get_template("template_relatorio_secundario.html")
    return tmpl.render(
