...
===============================
Projeção de IPCA da Anbima → `data/ipca.db`, tabela `IPCAProjetado`.
...
se precifica IPCA+ no mês corrente. Ver `lib/calc.py`.
...
CLI:
    python scripts/scrape_ipca_projetado_anbima.py
"""
...
SQL_UPSERT = """
INSERT OR REPLACE INTO IPCAProjetado (dtIPCAProjetado, vrProjecaoIPCA)
VALUES (?, ?)
"""
...
def Backup(log) -> Path | None:
    """Cópia do ipca.db antes de gravar. O histórico de IPCAProjetado NÃO é
    reconstruível (a página da Anbima só mostra ~13 meses) — daí a rede."""
    origem = CaminhoBancoIpca()
    if not origem.exists():
        return None

    DIR_BACKUP.mkdir(parents=True, exist_ok=True)
    destino = DIR_BACKUP / f"ipca_{datetime.now():%Y%m%d_%H%M%S}.db"
    shutil.copy2(origem, destino)

    antigos = sorted(DIR_BACKUP.glob("ipca_*.db"))[:-MAX_BACKUPS]
...
def Gravar(registros: list[tuple[str, float]], log) -> int:
    conn = ObterBancoIpca()
    try:
        antes = conn.execute("SELECT COUNT(*) FROM IPCAProjetado").fetchone()[0]
        conn.executemany(SQL_UPSERT, registros)
        conn.commit()
        depois = conn.execute("SELECT COUNT(*) FROM IPCAProjetado").fetchone()[0]
    finally:
        conn.close()
