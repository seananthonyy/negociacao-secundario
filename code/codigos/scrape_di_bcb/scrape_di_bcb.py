...
CLI:
    python scripts/scrape_di_bcb.py
    python scripts/scrape_di_bcb.py --start 2020-01-01 --end 2020-12-31
"""
...
SQL_UPSERT = """
INSERT OR REPLACE INTO DiHistorico (dtReferencia, vrTaxaDiAnual, vrTaxaDiDiaria)
VALUES (?, ?, ?)
"""
...
def ResolverJanela(args: argparse.Namespace, conn, log) -> tuple[date, date]:
    fim = date.fromisoformat(args.end) if args.end else date.today()

    if args.start:
        return date.fromisoformat(args.start), fim

    ultima = conn.execute("SELECT MAX(dtReferencia) FROM DiHistorico").fetchone()[0]
    if ultima:
...
def Principal() -> None:
    ...
    try:
        ...
        try:
            ...
            antes = conn.execute("SELECT COUNT(*) FROM DiHistorico").fetchone()[0]
            conn.executemany(SQL_UPSERT, registros)
            conn.commit()
            depois = conn.execute("SELECT COUNT(*) FROM DiHistorico").fetchone()[0]
        finally:
            conn.close()
